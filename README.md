...
可以将相连的直线线段或**仅转向一次的(S形)曲线**替换为道路中国标线类型的方块，使用 `/cs repline <类型>` 将脚下相连的线段替换成道路中国的标线。

`<类型>`：单白实线`w` 单黄实线`y` 双白实线`dw` 双黄实线`dy`； 单白虚线`wd` 单黄虚线`yd` 双白虚线`dwd` 双黄虚线`dyd`

转向多于一次的反向曲线请**拆分成若干S型曲线**方可使用本工具
